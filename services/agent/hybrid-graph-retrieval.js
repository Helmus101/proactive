...
async function coreDownRanking(nodeRows = [], retrievalPlan = {}, limit = 80) {
  ...
  for (let depth = 1; depth <= 2 && frontier.length; depth++) {
    ...
    for (const edge of edges || []) {
      ...
      const edgeWeightBonus = EDGE_WEIGHTS[edge.edge_type] || 1.0;
      const base = (Math.max(0.2, 0.95 - (depth * 0.18)) + (Number(edge.weight || 0) * 0.06) + (Number(edge.evidence_count || 0) * 0.02) + termBoost) * edgeWeightBonus;
...
async function buildHybridGraphRetrieval({
  ...
  if (recursionDepth > 0) {
    const anchors = fused.filter(r => (r.layer === 'core' || r.layer === 'insight') && r.base_score > 0.4).slice(0, 3);
    if (anchors.length > 0) {
      const subQueries = anchors.map(a => `${a.text} ${query}`.slice(0, 200));
      const secondPass = await buildHybridGraphRetrieval({
        query,
        options: {
          ...options,
          semantic_queries: subQueries,
          mode: 'chat'
        },
        seedLimit: 5,
        hopLimit: 1,
      ...
      // Merge results from second pass into fused results using RRF
      if (secondPass && secondPass.evidence) {
        const secondPassRanking = secondPass.evidence.map((ev, idx) => ({
          ...ev,
          key: ev.id,
          base_score: ev.score,
          match_reason: `recursive:${ev.reason}`
        }));
        const combined = reciprocalRankFusion([fused, secondPassRanking]);
        fused.length = 0;
        fused.push(...combined);
      }
    }
